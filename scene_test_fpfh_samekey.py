...
def main(args):
    print "3aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
    scenes_dir = args[1]
    base_path = args[2]
    ...
    for s in range(0, scenes_count):
        ...
        #reader_dummy = PlyReader_fpfh.PlyReader()
        #reader_dummy.read_ply(scene_path, num_samples=num_samples, add_noise=False, noise_std=0.5, noise_prob=0, noise_factor=0,
        #                   rotation_axis=[0, 0, 1], rotation_angle=utils.rad(0), 
        #                   sampling_algorithm=SampleAlgorithm.Uniform)
        
        #reader_dummy.set_variables(l=-1, patch_dim=patch_dim, filter_bad_samples=False, filter_threshold=50, use_point_as_mean=False)
