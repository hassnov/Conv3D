    ...
    for s in range(0, scenes_count):
        ...
        for model_num in range(len(model_paths)):
            ...
            with tf.Graph().as_default() as graph:
                ...
                for ratio_i, ratio1 in enumerate(ratios):
                    with open("results_stan_5models_{1}_clustering{0}.txt".format(ratio1, save_prefix), "a") as myfile:
                        ...
                        myfile.close()
                    with open("precision_stan_5models_{1}_clustering{0}.txt".format(ratio1, save_prefix), "a") as myfile:
                        myfile.write("{0:.4f}".format(correct_arr[ratio_i]) + '\n')
                        myfile.close()
                    with open("recall_stan_5models_{1}_clustering{0}.txt".format(ratio1, save_prefix), "a") as myfile:
