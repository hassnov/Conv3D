...
def main(args):
    ...
    for s in range(9, scenes_count):
        ...
        for model_num in range(len(model_paths)):
            ...
            num_cf, _, _ = utils.num_corresponding_features(reader.samples, reader_scene_samples_full, reader_scene.data, trans_mat, support_radii)
            ...
            with tf.Graph().as_default() as graph:
                ...
                for ratio_i, ratio1 in enumerate(ratios):
                    with open("results_ret_5models_{1}_clustering{0}.txt".format(ratio1, save_prefix), "a") as myfile:
                        ...
                        myfile.close()
                    with open("precision_ret_5models_{1}_clustering{0}.txt".format(ratio1, save_prefix), "a") as myfile:
                        myfile.write("{0:.4f}".format(correct_arr[ratio_i]) + '\n')
                        myfile.close()
                    with open("recall_ret_5models_{1}_clustering{0}.txt".format(ratio1, save_prefix), "a") as myfile:
...
"""
"""
main(["", r"/home/hasan/hasan/workspace/Conv3d/retrieval_dataset/3D models/Stanford/Retrieval",
