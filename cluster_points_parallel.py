...
class ClusterPoints:
    ...
    def process_all_rotations(self,dir_temp, X, global_i, ref_points, num_rotations, r, noise_i, relR_i, augs):
        z_axis = np.array([0, 0, 1])
        thetas = np.asarray(np.arange(-np.pi, np.pi, (2*np.pi)/num_rotations))
        thetas = thetas[0:num_rotations]
        num_cores = multiprocessing.cpu_count()
        #print "num_cores: ", num_cores
        Parallel(n_jobs=num_cores)(delayed(process_rotation)(dir_temp, X, rot_i, global_i, ref_points, num_rotations, r, noise_i, relR_i, augs,thetas, z_axis, self.patch_dim) for rot_i in range(num_rotations))
        #print Parallel(n_jobs=num_cores)(delayed(np.sqrt)(i) for i in range(10))
        
    
    def create_dataset(self, dir_temp='temp/', num_rotations=40, noises=[0, 0.3, 0.5], relRs=[0.05, 0.07]):
        z_axis = np.array([0, 0, 1])
        #### saving data set after clustering
        print "creating dataset..."
        X = np.zeros((num_rotations * len(noises) * len(relRs), self.patch_dim, self.patch_dim, self.patch_dim, 1), np.int8)
        ...
        for file1 in self.files_list:
            ...
            augs = np.arange(num_rotations * len(noises) * len(relRs)).reshape((len(noises), len(relRs), num_rotations))
            ...
            for _, samplept in enumerate(pc_samples):
                ...
                for noise_i, noise_level in enumerate(noises):
                    for relR_i, relR in enumerate(relRs):
                        ...
                        #Y.fill(0)
                        ref_points, _ = self.get_ref_points(samplept, r, noise_level=noise_level, model_resolution=mr)
                        
                        self.process_all_rotations(dir_temp, X, global_i, ref_points, num_rotations, r, noise_i, relR_i, augs)
...
def process_rotation(dir_temp, X, rot_i, global_i, ref_points, num_rotations, r, noise_i, relR_i, augs, thetas, z_axis, patch_dim):
    aug_num = augs[noise_i, relR_i, rot_i]
    theta = thetas[rot_i]
