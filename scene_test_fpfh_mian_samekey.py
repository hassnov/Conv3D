...
def main(args):
    ...
    for s in range(2, scenes_count):
        ...
        for model_num in range(len(model_paths)):
            ...
            for ratio_i, ratio1 in enumerate(ratios):
                with open("results_fpfh_mian{0}.txt".format(ratio1), "a") as myfile:
                    ...
                    myfile.close()
                with open("precision_fpfh_mian{0}.txt".format(ratio1), "a") as myfile:
                    myfile.write("{0:.4f}".format(correct_arr[ratio_i]) + '\n')
                    myfile.close()
                with open("recall_fpfh_mian{0}.txt".format(ratio1), "a") as myfile:
